...
public class InsiteTest extends SettingsDriver {
    ...
    private static final String LOGIN = "kot4@yopmail.com";
    private static final String PASSWORD = "Dv9aA8";
    ...
    public void createInsite() {
        ...
        step("Step 3. Ввести данные для авторизации", () -> {
            step("Step 3.1 Ввести логин - " + LOGIN, () -> $(byTestId("Email input")).setValue(LOGIN));
            step("Step 3.2 Ввести пароль - " + PASSWORD, () -> $(byTestId("Password input")).setValue(PASSWORD));
                ...
                () -> {
                    ...
                    step("Step 5.2 Видеопроигрыватель с элементами", () -> {
                        step("Step 5.2.1 Полоса прокрутки и отображения таймлайна",
                                () -> $(byClass("sc-fzoiQi ilyUIH")).shouldHave(Condition.hidden));
                    });
                    ...
                    step("Step 5.3 Блок информации о видео с элементами: ", () -> {
                        step("Step 5.3.1 Раскрывающийся список \"Профиль пользователя tttt@yopmail.com\" (по умолчанию скрыт)",
                                () -> $(byClass("sc-fzoMdx iKwhSE")).shouldBe(visible).click());
                        ...
                        step("Step 5.3.2 Номер видео",
                                () -> $(byClass("MuiGrid-root MuiGrid-container MuiGrid-align-items-xs-center"))
                    ...
                    step("Step 5.4 Блок \"Задания\" (справа от видеопроигрывателя) с элементами:", () -> {
                        step("Step 5.4.1 Заголовок \"Задания\"",
                                () -> $(byClass("MuiTypography-root MuiTypography-h5")).shouldHave(Condition.text("Задания")));

                        step("Step 5.4.2 Поле \"Поиск по транскрипту\"",
                                () -> $(byClass("MuiGrid-root sc-fznLPX kfIeLV MuiGrid-container MuiGrid-align-items-xs-center"))
                                        .shouldBe(visible));
                    });
        ...
        step("Step 6 Нажать кнопку \"Создать инсайт\" на панели видеопроигрывателя", () -> {
            step("Step 6.1.1 Нажать на видео для того что бы появилась кнопка 'Создать инсайт'",
                    () -> $(byClass("sc-fznxKY kfRyXm")).click());

            step("Step 6.1.3 Нажать кнопку 'Создать инсайт'",
                    () -> $(byClass("sc-fzoXzr kcfuJb")).shouldBe(visible).click());

        });

        step("Step 7 Ввести в поле внутри окна инсайта текст \"Инсайт\"",
                () -> $(byClass("MuiInputBase-input MuiInputBase-inputMultiline")).setValue("INSITE"));

        step("Step 8 Нажать кнопку \"Добавить\"",
                () -> $(byClass("MuiButton-label")).shouldBe(enabled).click());
    ...
    public void editInsite() {
        ...
        step("Step 6. В списке инсайтов в строке с названием \"Инсайт\" нажать кнопку \"Редактировать\"",
                () -> $$x("//button[contains(@title,'Редактировать')]")
                        .first().click());
        ...
        step("Step 7 Изменить текст в поле на \"Инсайт для удаления\"\n" +
                ...
                () -> {
                    step("Step 7.1 Ввести в поле \"Инсайт для удаления\"",
                            () -> $(byClass("MuiInputBase-input MuiOutlinedInput-input")).setValue("Insite for delete"));
        ...
        step("Step 8 Выйти из профиля", () -> $("[ng-tr='PRV.PRV73']").click());
        step("Step 9 Ожидания появления главной страницы", () -> $("[ng-tr='NLHOM.NLHOM3']")).shouldBe(visible);
    }
